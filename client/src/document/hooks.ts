...
export function useExplainSnippet(doc: Doc | undefined) {
  useEffect(() => {
    const elements = [
    ...
    elements.forEach((element) => {
      ...
      button.addEventListener("click", async () => {
        ...
        element.insertAdjacentElement("beforeend", explanation);
        created.push(explanation);
        ...
        try {
          const res = await fetch("/api/v1/chat/explain", {
            method: "POST",
            headers: {
...
${"```"}`,
          ...
          footer.innerText = "This is an experiment, generated by GPT-3.";
          explanation.insertAdjacentElement("beforeend", footer);
          created.push(footer);
        } catch {
          window.clearInterval(interval);
          explanation.innerText = "Error! Try again.";
          button.disabled = false;
        }
      });
    });
